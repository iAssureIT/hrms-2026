    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {loading ? (
          ...
          holidays.map((h, i) => (
            <div key={i} className="group p-4 rounded-xl border border-transparent hover:border-blue-100 hover:bg-blue-50/30 transition-all flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex flex-col items-center justify-center w-12 h-12 rounded-xl bg-gray-50 border border-gray-100 text-gray-800 font-black group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {h.locations?.map((loc, idx) => (
                      <span key={idx} className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tight ${
                        loc === "Global" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"
                      }`}>
                        {loc}
                      </span>
                    ))}
