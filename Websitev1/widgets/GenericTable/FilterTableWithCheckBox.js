...
const GenericTable = ({
    ...
    handleAction,
    customDownloadURL,
    // ── Checkbox props (optional) ──
    checkboxSelection,      // boolean: enable checkbox column
    selectedRows,           // array of selected IDs from parent
    onRowSelect,            // fn(id) called when single row checkbox changes
    onSelectAll,            // fn(e) called when header checkbox changes
    rowIdKey = "_id",       // which field to use as row ID (default: _id)
}) => {
            ...
            <table  className={`table-auto text-base bottom border-separate border-spacing-y-2 w-full dark:w-full`}> */}
                    <div className="relative w-full mt-3">
                        <div className="table-responsive table-container overflow-x-auto">
                            <table className="min-w-full table-fixed text-base bottom  border-separate border-spacing-y-1 w-full dark:w-full leading-tight">
                                <thead
                                    className="bg-[#f8fafc]"
                                >
                                    ...
                                    {tableData && tableData.length > 0 ? (
                                        tableData.map((value, i) => {
                                            ...
                                            return (
                                                <tr
                                                    key={i}
                                                    className="border-b border-gray-200 text-[13px] even:bg-white text-[#000] font-normal"
                                                >
                                                    {/* ── Checkbox cell: only rendered if checkboxSelection is enabled ── */}
                                                    {checkboxSelection && (
                                                        <td className="px-4 py-2 text-center">
                                                            <input
                                                                type="checkbox"
                                                                className="w-4 h-4 rounded"
                                                                // Check against the rowIdKey field (default: _id)
                                                                checked={selectedRows?.includes(value[rowIdKey]) || false}
                                                                onChange={() => onRowSelect && onRowSelect(value[rowIdKey])}
                                                            />
                                                        </td>
                                                    )}
                                                    ...
                                                    {Object.keys(tableHeading).map((key, index) => {
                                                        if (key === "employeeName") {
                                                            return (
                                                                <td key={index} className="px-2 md:px-2 py-2">
                                                                    ...
                                                                    >
                                                                        <div className="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center shrink-0 shadow-sm">
                                                                            {value.profilePhoto ? (
                                                                        ...
                                                                        <div className="flex flex-wrap md:flex-nowrap mx-3 gap-2 items-center justify-center">
                                                                            {loggedInRole === "admin" &&
                                                                                ...
                                                                                "Add Center Incharge" && (
                                                                                    ...
                                                                                    >
                                                                                        <FaPlus
                                                                                            className="text-gray-500 px-1 py-0.5 cursor-pointer rounded-sm hover:border-gray-400 hover:text-gray-400"
                                                                                            size={"1.3rem"}
                                                                                            onClick={() => {
                                                                                                window.open(
                                                                                                    "/" +
                                                        ...
                                                        return (
                                                            <td
                                                                className={`px-2 py-2 text-wrap whitespace-normal break-words max-w-xs overflow-hidden ${amountArr.includes(key)
