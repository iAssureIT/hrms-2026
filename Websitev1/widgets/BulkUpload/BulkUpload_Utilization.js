...
const BulkUpload = (props) => {
  ...
  const bulkUpload = async () => {
    console.log(
      inputFileData.map((r, i) => ({
        row: i + 1,
        center: r.centerName,
      }))
    );
    var invalidData = [];
    const firstValidRow = inputFileData.find(
      r =>
        r.centerName &&
        r.centerName !== "-" &&
        r.centerName.toString().trim() !== ""
    );
    var centerFirstName = (firstValidRow?.centerName || "")
      .toString()
      .trim()
      .toLowerCase();

    console.log("centerFirstName", inputFileData[0]?.centerName);

    console.log("inputFileData", inputFileData[0]);

    inputFileData.forEach(function (item) {
      console.log(
        "centerFirstName !== item?.centerName.trim()",
        centerFirstName !== item?.centerName.trim()
      );
      console.log("centerFirstName", centerFirstName);
      console.log("item?.centerName.trim()", item?.centerName.trim());


      const currentCenter = (item?.centerName || "")
        .toString()
        .trim()
        .toLowerCase();

      if (currentCenter === "" || currentCenter === "-") { return; }

      if (centerFirstName !== currentCenter) {
        var remark = "All Center Names should be the same.";
        var invalidObject = { ...item, failedRemark: remark };
        invalidData.push(invalidObject);
      }
      console.log("CENTER USED FOR API:", currentCenter );
    });


    if (invalidData.length > 0) {
      new swal({
        title: "All Center Names should be the sameeee.",
        text: "Please verify Data!",
      });
    } else {
      console.log("Api center value --> ",centerFirstName);
      
      var centerDetails = await axios.get(
        "/api/centers/get/center/name" + centerFirstName
      );
