                  ...
                  <div className="w-3/5 flex flex-col justify-center px-3 text-[13px]">
                    ...
                    <div className="relative w-full">
                      <button
                        ...
                        className={`stdSelectField font-normal pl-2 pr-8 py-1.5 w-full rounded-md border border-gray-300 text-sm text-left truncate relative bg-white ${
                          dataType ? "text-black" : "text-gray-400"
                        }`}
                      >
                        {dataType === "cumulative"
                          ? "Cumulative"
                          : dataType === "priorToFinancialYear"
                            ? `Prior to ${year}`
                            : dataType === "current"
                              ? `Current (${year})`
                              : "Select Duration"}
                        <svg
                          className={`w-4 h-4 absolute right-2 top-1/2 -translate-y-1/2 transition-transform ${
                            dataTypeDropdownOpen ? "rotate-180" : ""
                          }`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                      ...
                      {dataTypeDropdownOpen && (
                        <div className="absolute z-[100] min-w-full w-max bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto overflow-x-hidden">
                          <div
                            onClick={() => {
                              setDataType("cumulative");
                              setDataTypeDropdownOpen(false);
                            }}
                            className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm text-black font-normal whitespace-nowrap"
                          >
                            Cumulative
                          </div>
                          <div
                            onClick={() => {
                              setDataType("priorToFinancialYear");
                              setDataTypeDropdownOpen(false);
                            }}
                            className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm text-black font-normal whitespace-nowrap"
                          >
