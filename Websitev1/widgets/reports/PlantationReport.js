...
function PlantationReport() {
  ...
  const tableHeading = {
    ...
    "plantationDetails[0].speciesDetails[0].speciesName":
    ...
    "plantationDetails[0].speciesDetails[0].avgDiameter":
      "Avg. Diameter (inch)",
  ...
  useEffect(() => {
    ...
    const speciesArray =
      SPECIES_NAMES?.split(",")?.map((event) => event?.trim()) || [];
  ...
  const downloadExcel = async () => {
    try {
        ...
        events.forEach((event, eventIdx) => {
          ...
          speciesList.forEach((species, spIdx) => {
            ...
            row.push(
              ...
              species.avgDiameter
                ? parseFloat(species.avgDiameter).toFixed(1)
                : "",
              species.yeild || 0,
              formatToINR(species.income || 0),
              // event.sitePhotos?.length > 0 ? "Photo Available" : "",
              Array.isArray(event.sitePhotos)
                ? event.sitePhotos.map((p) => p.uri).join(", ")
                : "--NA--",
            );
      ...
      fullData.forEach((record) => {
        const totalRows =
          record.plantationDetails?.reduce(
            (sum, e) => sum + (e.speciesDetails?.length || 1),
            0,
          ) || 1;
        if (totalRows > 1) {
          for (let i = 0; i < 16; i++) {
            merges.push({
              s: { r: currentRow, c: i },
              e: { r: currentRow + totalRows - 1, c: i },
            });
          }
        }
      ...
      fullData.forEach((record) => {
        record.plantationDetails?.forEach((event) => {
          ...
          if (speciesCount > 1) {
            merges.push({
              s: { r: currentRow, c: 16 },
              e: { r: currentRow + speciesCount - 1, c: 16 },
            });
            merges.push({
              s: { r: currentRow, c: 24 },
              e: { r: currentRow + speciesCount - 1, c: 24 },
            }); // Photos
          }
      ...
      XLSX.writeFile(
        wb,
        `Plantation_Report_${moment().format("DD-MMM-YYYY")}.xlsx`,
      );
              ...
              <div className="">
                ...
                <div className="relative mt-2 rounded-md shadow-sm text-gray-500">
                  <button
                    type="button"
                    onClick={(e) =>
                      toggleDropdown(e, setBlockDropdownOpen, blockDropdownOpen)
                    }
                    className="stdSelectField pl-3 text-left flex justify-between items-center"
                  >
                          ...
                          <td className="px-2 py-3 border border-grayTwo text-right">
                            {formatNumberToCommas(
                              tableData
                                .flatMap(
                                  (r) =>
                                    r.plantationDetails?.[0]?.speciesDetails ||
                                    [],
                                )
                                .reduce(
