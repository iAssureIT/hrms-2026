      ...
      <div className="mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div className="flex items-baseline gap-3">
            <h1 className="text-2xl font-normal text-gray-800 tracking-tight">Leave Management</h1>
        ...
        <div className="bg-white border-t-[3px] border-[#00a65a] shadow-sm flex flex-col mb-10">
          ...
          <div className="border-b border-gray-100 flex flex-col md:flex-row justify-between items-center bg-white px-2">
              ...
              {["Pending Requests", "Approved", "Rejected", "Leave Ledger", "All Records"].map((tab) => (
                <button
                  ...
                  className={`px-5 py-3 text-[12px] font-bold transition-all relative whitespace-nowrap border-r border-gray-100 ${
                    activeTab === tab ? "bg-white text-gray-800 border-t-2 border-t-[#00a65a] -mt-[2px]" : "bg-gray-50 text-gray-400 hover:bg-gray-100"
                  }`}
                >
                  {tab}
            ...
            <div className="p-3 hidden md:block">
              <h3 className="text-sm font-bold text-gray-800 uppercase tracking-tight opacity-50">{activeTab}</h3>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse border-b border-gray-200">
              <thead className="bg-[#f9f9f9]">
                <tr>
                  ...
                  <th className="px-5 py-3 text-[12px] font-bold text-gray-700 border-b border-gray-200 border-r border-gray-100 last:border-r-0">
                    <div className="flex items-center justify-between">
                      {activeTab === "Leave Ledger" ? "Action & Balance" : "Statement of Reason"}
                      <MdFilterList className="text-gray-300" />
                    </div>
                  </th>
                  <th className="px-5 py-3 text-[12px] font-bold text-gray-700 border-b border-gray-200 border-r border-gray-100 last:border-r-0">
                    <div className="flex items-center justify-between">
                      {activeTab === "Leave Ledger" ? "Ref" : "Submission Datetime"}
                      <MdFilterList className="text-gray-300" />
                    </div>
                  </th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="5" className="px-5 py-20 text-center text-gray-400 text-xs italic">Loading records...</td>
                  </tr>
                ) : (activeTab === "Leave Ledger" ? ledger : filteredLeaves).length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-5 py-20 text-center text-gray-400 text-xs italic">No data available in table</td>
                  </tr>
                ) : activeTab === "Leave Ledger" ? (
                  ledger.map((item) => (
                    <tr key={item._id} className="hover:bg-[#f5f5f5] transition-colors border-b border-gray-100">
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div>
                          <div className="text-xs font-bold text-gray-700">{item.employeeId?.employeeName || "Unknown"}</div>
                          <div className="text-[10px] text-gray-400 uppercase">{item.employeeId?.employeeID || "N/A"}</div>
                        </div>
                      </td>
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div className="text-[11px] font-bold text-[#00a65a]">{item.leaveTypeId?.leaveTypeName || "Leave"}</div>
                        <div className="text-[10px] text-gray-500">{moment(item.createdAt).format("DD MMM YYYY")}</div>
                      </td>
                      <td className="px-5 py-3 border-r border-gray-100 text-xs">
                        <span className={item.days > 0 ? "text-[#00a65a] font-bold" : "text-[#dd4b39] font-bold"}>
                          {item.days > 0 ? "+" : ""}{item.days} ({item.transactionType})
                        </span>
                        <div className="text-[10px] text-gray-400 mt-1">Bal After: {item.balanceAfter}</div>
                      </td>
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div className="text-xs text-gray-500 italic max-w-[200px] truncate">{item.remarks}</div>
                      </td>
                    </tr>
                  ))
                ) : (
                  filteredLeaves.map((leave) => (
                    <tr key={leave._id} className="hover:bg-[#f5f5f5] transition-colors border-b border-gray-100">
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div className="text-xs font-bold text-gray-700">{leave.employeeId?.employeeName || "Unknown"}</div>
                        <div className="text-[10px] text-gray-400">{leave.employeeId?.employeeID || "N/A"}</div>
                      </td>
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div className="text-xs font-bold text-gray-600 mb-1">{leave.leaveTypeId?.leaveTypeName || "Leave"}</div>
                        ...
                        <div className="text-[10px] text-gray-400 mt-1 uppercase">{leave.totalDays} Days</div>
                      </td>
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div className="text-xs text-gray-500 italic line-clamp-2 max-w-[250px]">"{leave.reason}"</div>
                      </td>
                      <td className="px-5 py-3 border-r border-gray-100">
                        <div className="text-[11px] text-gray-400 mb-2">{moment(leave.createdAt).format("DD MMM YYYY")}</div>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-tight text-white ${
                          leave.status === 'APPROVED' ? 'bg-[#00a65a]' : leave.status === 'REJECTED' ? 'bg-[#dd4b39]' : 'bg-[#f39c12]'
                        }`}>
                          {leave.status}
                        </span>
                      </td>
          ...
          <div className="p-4 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="text-xs text-gray-700">
              Showing 1 to {(activeTab === "Leave Ledger" ? ledger : filteredLeaves).length} of {(activeTab === "Leave Ledger" ? ledger : filteredLeaves).length} entries
            </div>
            <div className="flex border border-gray-300 rounded-sm overflow-hidden">
              <button className="px-3 py-1.5 text-xs text-gray-500 hover:bg-gray-50 border-r border-gray-300">Previous</button>
              <button className="px-3 py-1.5 text-xs bg-[#3c8dbc] text-white border-r border-gray-300">1</button>
              <button className="px-3 py-1.5 text-xs text-gray-500 hover:bg-gray-50">Next</button>
            </div>
          </div>
        </div>
      </div>
