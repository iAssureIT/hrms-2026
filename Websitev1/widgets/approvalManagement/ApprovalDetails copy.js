...
const ApprovalDetails = () => {
  ...
  const getAuthRole = (roles) => {
    const authorities = approvalData?.approvalAuthourities?.map(auth => auth.approvalAuthRole) || [];
    ...
    axios
      ...
      .then(async (response) => {
        ...
        if (userDetails) {
          ...
          //   approvalAuthName
          // );
          const authRole = await getAuthRole(parseAuthRole);
          console.log("authRole-------------------------------------",authRole)
          setMatchedRoles(authRole);
          if (authRole.length === 1) {
            setSelectedRole(authRole[0]);
            setApprovalAuthRole(authRole[0]);
          }
  ...
  const isUserAuthourised = (approvalAuthName, approvalAuthRole) => {
    ...
    approvalData?.approvalAuthourities?.forEach((authority, index) => {
      if ( authority.approvalAuthName === approvalAuthName &&
    ...
    if (authorizedIndexes.length > 0) {
      ...
      if(approvalAuthRole){
        ...
        //   let authorizedIndex = authorizedIndexes[i];
          ...
          const authorizedIndex = authorities.findIndex(
            (auth) =>  auth.approvalAuthName === approvalAuthName && auth.approvalAuthRole === approvalAuthRole
          );
          const currentLevelStatus = approvalData.approvalAuthourities?.[currentIndex]?.status;
          const nextLevelStatus = approvalData.approvalAuthourities?.[currentIndex + 1]?.status;
          const prevLevelStatus = approvalData.approvalAuthourities?.[currentIndex - 1]?.status;
          console.log("prevLevelStatus",prevLevelStatus)
          console.log("nextLevelStatus",nextLevelStatus)
          ...
          // if (currentIndex === 0 || approvalData.approvalAuthourities[currentIndex - 1]?.status === "approved") {
          //   if (
          //     approvalData.approvalAuthourities[currentIndex + 1]?.status !== "approved" &&
          //     approvalData.approvalAuthourities[currentIndex + 1]?.status !== "rejected"
          //   ) {
          ...
          // }else if (currentIndex > 0 && prevLevelStatus === "rejected") {
          //   showActionButtons = false; 
          // }else if (currentIndex > 0 && prevLevelStatus === "pending") {
          //   showActionButtons = false; 
          // }else{
          //   showActionButtons = true; 
          // }
      ...
      <div className="box border-2 rounded-md shadow-md" id="pdf-content">
        <div className="uppercase text-xl font-semibold">
          <div className="border-b-2 border-gray-300 flex justify-between">
            <h1 className="heading">Approval Details</h1>
        ...
        <div className="px-10 py-6">
          ...
          {console.log(
            ...
            isUserAuthourised(approvalAuthName, approvalAuthRole),"====",  approvalAuthName,"====",
            approvalAuthRole,
          )}
          {isUserAuthourised(approvalAuthName, approvalAuthRole) ||
          loggedInRole === "admin" ? (
            <div className="border-t-2">
              ...
              <div className="flex gap-10 justify-between mt-5 mb-6 w-full">
                <div className="w-1/2">
                ...
                {console.log("isUserAuthourised(approvalAuthName, approvalAuthRole)?.showActionButtons",isUserAuthourised(approvalAuthName, approvalAuthRole)?.showActionButtons,"----",isUserAuthourised(approvalAuthName, approvalAuthRole)?.showActionButtons || loggedInRole !== "admin" )}
                {isUserAuthourised(approvalAuthName, approvalAuthRole)?.showActionButtons && loggedInRole !== "admin" ? (
                  <div className="w-1/2 justify-center hide-in-pdf">
                     <div>
                      {matchedRoles.length > 1 
                      ? (
                        <div className="mx-auto">
                          <label className="py-2 flex justify-center">Select Role:</label>
                          <div className="flex justify-center">
                            <select 
                            name="approvalAuthRole"
                            id="approvalAuthRole"
                            className={`
                              ${approvalAuthRole ? "text-black stdSelectField w-1/2" : "text-gray-400 w-1/2 stdSelectField"}
                              `}
                                value={selectedRole} 
                                onChange={(e) => {
                      ...
                      matchedRoles.length === 1 ? (
                        <div className="flex justify-center">
                          <p>Approve This Activity as <strong>{matchedRoles[0]}</strong></p>
                        </div>
                      ) 
                      : null}
                    </div>
