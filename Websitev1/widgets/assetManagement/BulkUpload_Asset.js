...
import ProgressBar from "@/widgets/BulkUpload/ProgressBar.js";
...
import "@/widgets/BulkUpload/BulkUpload.css";
import { IoCloseCircleSharp } from "react-icons/io5";
import { FaRegFileAlt } from "react-icons/fa";
import { FaSpinner } from "react-icons/fa6";
import ls from "localstorage-slim";

const BulkUpload_Asset = (props) => {
  ...
  useEffect(() => {
    // Try localstorage-slim first (as used in parent)
    const user = ls.get("userDetails", { decrypt: true }) || ls.get("userDetails");
    if (user) {
      setUser_id(user.user_id || user.userId || user._id || user.ID);
    } else {
      ...
      if (userDetails) {
        try {
          ...
          const parseUser_id = userDetailsParse.user_id || userDetailsParse.userId || userDetailsParse.ID || userDetailsParse._id;
  ...
  const bulkUpload = async () => {
    console.log("inputFileData", inputFileData[0]);
        ...
        if (i === endLmt - 1 && i !== totalrows && chunkData.length > 0) {
          ...
          await axios({
            ...
            .catch((error) => {
              ...
              const errorMsg = error.response?.data?.error || error.message || "Something unexpected happened!";
              new swal({
      ...
      {props.fileDetails ? (
        <div>
          <div className="flex mb-4 capitalise">
            <button
              className={`px-6 py-2 hover:bg-gray-200 transition-all ${activeTab === "active"
                ? "text-green bg-white border-green border-b-2 font-semibold"
                : "text-gray-700"
                }`}
            ...
            <button
              className={`px-6 py-2 flex gap-2 justify-center items-center hover:bg-gray-200 transition-all ${activeTab === "failure"
                ? "text-green bg-white border-green border-b-2 font-semibold"
                : "text-gray-700"
                }`}
          ...
          <div>
            {activeTab === "active" ? (
              <div
                ...
                className="tab-pane-custom show-content"
              >
                <h5>
...
export default BulkUpload_Asset;
