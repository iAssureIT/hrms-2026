...
const BulkUpload = (props) => {
  ...
  const handleFile = (file) => {
    ...
    reader.onload = ({ target: { result } }) => {
      ...
      // e.g. { "Full Name": "employeeName", "Center": "centerName" }
      const headerMapping = {};
      if (props.goodRecordsHeading) {
        Object.entries(props.goodRecordsHeading).forEach(([key, label]) => {
          headerMapping[label.toLowerCase().trim()] = key;
        });
      }
        ...
        if (record !== undefined && record.length > 0) {
          ...
          for (let k in header) {
            ...
            if (record[k] === undefined) {
              ...
              rowObj[cleanHeader] = record[k];
            }
          }
          documentObj.push(rowObj);
        }
      }
