...
const AddApprovalLevel = () => {
  ...
  useEffect(() => {
    if (params._id) {
      axios
        .get("/api/approvalLevels/get/one/" + params._id)
        .then((program) => {
  ...
  const getAuthourityData = () => {
    axios
      ...
      .catch((error) => {
        console.log("Error while getting approval List => ", error);
      });
  };
  ...
  const handleSumbit = (e) => {
    ...
    if (validation()) {
      ...
      if (params._id) {
        ...
        axios
          .put("/api/approvalLevels/put/update", formValues)
          .then((updatedProduct) => {
            if (updatedProduct.data) {
              setRunCount((count) => count + 1);
              if (updatedProduct.data.data.modifiedCount > 0) {
                Swal.fire(" ", "Activity Activity Approval level  details updated successfully!!");
                ...
                Swal.fire(
                  " ",
                  "Activity Approval level  details was not changed hence no update!!"
                );
              }
            }
          })
        ...
        axios
          .post("/api/approvalLevels/post", formValues)
          .then((response) => {
            setRunCount((count) => count + 1);
