...
const EmployeeLeaveLedger = () => {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Leave_Ledger_${employee?.employeeName || "Employee"}_${month}_${year}.csv`,
    );
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight pl-1">
                {employee?.employeeName || "Employee"}'s Leave{" "}
                <span className="text-[#3c8dbc] font-black">Ledger</span>
              </h1>
              <p className="text-sm text-gray-500 pl-1 mt-1 font-light">
                {moment()
          ...
          >
            <div className="relative group">
              <FaDownload
                className="cursor-pointer text-[#00a65a] hover:text-[#008d4c] border border-[#00a65a] p-1.5 hover:border-[#008d4c] rounded text-[34px] transition-all active:scale-95 shadow-sm"
                onClick={exportToCSV}
              />
            </div>
          </Tooltip>
        ...
        <div className="bg-white rounded-sm shadow-sm border-t-[3px] border-[#3c8dbc] mb-8 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-[11px] font-bold text-gray-600 uppercase border-b">
                  Leave Type
                </th>
                <th className="px-6 py-3 text-[11px] font-bold text-gray-600 uppercase border-b text-center">
                  Opening Balance
                </th>
                <th className="px-6 py-3 text-[11px] font-bold text-gray-600 uppercase border-b text-center">
                  Earned (Month)
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              <tr>
                ...
                <td className="px-6 py-4 text-sm text-center font-bold text-gray-500">
                  {summary?.earnedLeave?.opening || 0}
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-[#00a65a]">
                  {summary?.earnedLeave?.monthlyEarned || 0}
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-[#dd4b39]">
                  {summary?.earnedLeave?.monthlyUsed || 0}
                </td>
                <td className="px-6 py-4 text-center">
                  <span
                    className={`text-sm font-bold ${summary?.earnedLeave?.balance < 0 ? "text-red-600" : "text-gray-800"}`}
                  >
              ...
              <tr>
                ...
                <td className="px-6 py-4 text-sm text-center font-bold text-gray-500">
                  {summary?.compOff?.opening || 0}
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-[#3c8dbc]">
                  {summary?.compOff?.monthlyEarned || 0}
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-[#dd4b39]">
                  {summary?.compOff?.monthlyUsed || 0}
                </td>
                <td className="px-6 py-4 text-center">
                  <span
                    className={`text-sm font-bold ${summary?.compOff?.balance < 0 ? "text-red-600" : "text-gray-800"}`}
                  >
              ...
              <tr className={summary?.lop > 0 ? "bg-red-50" : ""}>
                <td
                  className={`px-6 py-4 text-sm font-bold ${summary?.lop > 0 ? "text-red-600" : "text-gray-700"}`}
                >
                  Loss of Pay (LOP)
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-gray-400">
                  ---
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-gray-400">
                  ---
                ...
                <td className="px-6 py-4 text-sm text-center font-bold text-[#dd4b39]">
                  {summary?.monthlyLopUsed || 0}
                </td>
                <td className="px-6 py-4 text-center">
                  <span
                    className={`text-sm font-bold ${summary?.lop > 0 ? "text-red-600" : "text-gray-400"}`}
                  >
                    {summary?.lop || 0}
                  </span>
                </td>
              ...
              <tr className="bg-blue-50/30 border-t-2 border-gray-100">
                <td className="px-6 py-4 text-sm font-extrabold text-[#3c8dbc]">
                  Net Balance (EL + CO - LOP)
                </td>
                <td className="px-6 py-4 text-sm text-center font-bold text-gray-400">
                  ---
                </td>
                ...
                <td className="px-6 py-4 text-center">
                  <span
                    className={`text-sm font-black ${summary?.totalBalance - summary?.lop < 0 ? "text-red-600" : "text-[#3c8dbc]"}`}
                  >
                    {(() => {
                      const net =
                        (summary?.totalBalance || 0) - (summary?.lop || 0);
                      if (net < 0)
        ...
        <div className="bg-white rounded-sm shadow-sm border-t-[3px] border-[#00a65a]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-[#f9f9f9]">
                <tr>
                  <th className="px-4 py-3 text-[11px] font-bold text-gray-700 uppercase border-b border-gray-200">
                    Date
                  </th>
                  <th className="px-4 py-3 text-[11px] font-bold text-gray-700 uppercase border-b border-gray-200">
                    Type
                  ...
                  <th className="px-4 py-3 text-[11px] font-bold text-gray-700 uppercase border-b border-gray-200 text-center">
                    Days
                  </th>
                  <th className="px-4 py-3 text-[11px] font-bold text-gray-700 uppercase border-b border-gray-200 text-center">
                    Balance
              ...
              <tbody>
                {filteredLedger.length === 0 ? (
                  <tr>
                    <td
                      colSpan="7"
                      className="px-4 py-20 text-center text-gray-400 italic text-xs"
                    >
                  ...
                  filteredLedger.map((tx) => (
                    ...
                    >
                      ...
                      <td
                        className={`px-4 py-4 text-center font-bold text-[11px] ${
                      ...
                      <td
                        className="px-4 py-4 text-[10px] text-gray-500 italic max-w-[200px] truncate"
                        title={tx.remarks}
                      >
                        {tx.remarks}
                      </td>
          ...
          <div className="p-4 border-t border-gray-50 bg-gray-50">
            <p className="text-[10px] text-gray-400 italic">
              * Note: Running balance represents the remaining balance for that
              specific leave type after the transaction.
            </p>
          </div>
        </div>
      </div>
