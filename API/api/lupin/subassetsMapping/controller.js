...
const SubassetsMapping = require("./model");
...
const ActivityMaster = require("../oneFieldModules/assetsManagement/model.js");
const ObjectId = require("mongodb").ObjectId;
// const Subassets = require("../");
const FailedRecords = require("../failedRecords/model.js");

// CREATE
exports.createSubassets = async (req, res) => {
  ...
  try {
    ...
    const existingsubCategorySizes = await SubassetsMapping.findOne({
      dropdownvalue: req.body.dropdownvalue,
      inputValue: req.body.inputValue,
    });

    if (existingsubCategorySizes) {
      return res
        .status(409)
        .json({ message: "Category and subCategorySizes already exists" });
    }

    const newItem = new SubassetsMapping({
      _id: new mongoose.Types.ObjectId(),
      dropdownvalue: req.body.dropdownvalue,
      inputValue: req.body.inputValue,
      dropdown_id: req.body.dropdown_id,
      category_imgUrl: req.body.category_imgUrl1,
      subcategoryImgUrl: req.body.subcategoryImg,
      subcategoryImgName: req.body.subcatimgName,
  ...
  async function processData() {
    ...
    for (var k = 0; k < excelData.length; k++) {
      ...
      if (uniqueCombinations.has(combinationKey)) {
        remark = "Duplicate Subassets details found in the file.";
      } else {
      ...
      if (remark === "") {
        ...
        if (remark === "") {
          ...
          if (subactivityExists.length === 0) {
            ...
            remark = "Subassets details already exist.";
          }
        }
      }
    ...
    if (validData.length > 0) {
      SubassetsMapping.insertMany(validData)
        .then((data) => {
...
var removeFailedRecords = async (fileName) => {
  return new Promise(function (resolve, reject) {
    SubassetsMapping.deleteMany({ fileName: fileName })
      .exec()
      .then((data) => {
        resolve(data);
      })
...
exports.filedetails = (req, res, next) => {
  var finaldata = {};
  SubassetsMapping.find({ fileName: req.params.fileName })
    .exec()
    .then((data) => {
...
// exports.updateSubassets = (req, res, next) => {
//   // console.log("Request Body:", req.body); // Log request body for debugging
//   SubassetsMapping.updateOne(
//     { _id: req.params.id },
//     {
//       $set: {
...
//     });
// };
exports.updateSubassets = async (req, res) => {
  try {
    const updateResult = await SubassetsMapping.updateOne(
      { _id: req.params.id },
      {
        $set: {
          dropdownvalue: req.body.dropdownvalue,
          ...
          dropdownLabel: req.body.dropdownLabel,
          inputLabel: req.body.inputLabel,
          category_imgUrl: req.body.category_imgUrl,
          subcategoryImgUrl: req.body.subcategoryImgUrl,
          subcategoryImgName: req.body.subcategoryImgName,
        },
      }
    );
    ...
    if (updateResult.modifiedCount > 0) {
      ...
      await SubassetsMapping.updateOne(
        { _id: req.params.id },
        {
          $push: {
            updateLog: {
      ...
      return res.status(200).json({
        success: true,
        message: "SubAssets updated successfully.",
      });
...
exports.getAllSubassetsWithLimits = (req, res, next) => {
  ...
  SubassetsMapping.countDocuments(query)
    .then((totalRecs) => {
      // console.log("totalRecs => ", totalRecs);

      let subactivityQuery = SubassetsMapping.find(query);
...
//     // const subactivity = await SubassetsMapping.find({ field1_id: ObjectId(req.params.field1_id) });
//     const subactivity = await SubassetsMapping.find(query);
//     const projectList =
...
exports.getfield3List = async (req, res) => {
  ...
  try {
    ...
    const subactivity = await SubassetsMapping.find(query);
    // const subactivity = await SubassetsMapping.find(
    //                       {
  ...
  try {
    ...
    const uniqueSubassetsList =
      subactivityList.length > 0
        ...
        : [];
    uniqueSubassetsList.sort((a, b) => a.inputValue.localeCompare(b.inputValue));
    console.log("uniqueSubassetsList", uniqueSubassetsList.length);

    res.status(200).json(uniqueSubassetsList);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
// READ ALL
exports.getAllSubassets = async (req, res) => {
  try {
    const subactivities = await SubassetsMapping.find();
...
exports.getOneSubassets = async (req, res) => {
  try {
    const subactivity = await SubassetsMapping.findOne({
      _id: req.params.id,
    });
...
// DELETE
exports.deleteSubassets = async (req, res) => {
  try {
    const deletedSubassets = await SubassetsMapping.findByIdAndDelete(
      req.params.id
    );
    if (!deletedSubassets)
      return res.status(404).json({ message: "Subassets not found" });
    res.json({ message: "Subassets deleted" });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};

// exports.updateDropdownSubassets = async (req, res) => {
//   const newdropdownvalue = req.body[0];
//   const dropdownId = req.params.id;

//   try {
//     const result = await SubassetsMapping.updateMany(
...
//           } else {
//             remark = "Subassets Details already exists.";
