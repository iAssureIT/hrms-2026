...
exports.create_approval = (req, res, next) => {
  // console.log("Post API req.body", req.body);
  // console.log("Post API req.body", req.body.approverLevel);
    ...
    .then((data) => {
      if (data) {
        ...
        approvals
          .save()
          .then((response) => {
            // console.log("Inserted value of Products => ", response);
            res.status(200).json({
              insertedLevel: response,
              success: true,
              message: "Utilization submitted Successfully.",
            });
          })
          .catch((err) => {
            console.log(err);
    ...
    .then((data) => {
      ...
      if (data.modifiedCount == 1 || data.modifiedCount == 1) {
        ...
        res.status(200).json({
          data,
          success: false,
          message: "Utilization are not modified.",
        });
      }
    })
    .catch((err) => {
...
exports.delete_approval = (req, res, next) => {
  ApprovalLevels.deleteOne({ _id: req.params.ID })
    ...
    .then((data) => {
      res.status(200).json({
        message: "Utilization Utilization deleted Successfully.",
      });
    })
    .catch((err) => {
      console.log(err);
