...
const CC = require("./model.js");

exports.create_cc = (req, res, next) => {
  getData();
  async function getData() {
    ...
    cc.save()
      .then((response) => {
        res.status(200).json({
          message: "CC submitted Successfully.",
...
exports.update_cc = (req, res, next) => {
  CC.updateOne(
    { _id: req.body.ID },
    {
      $set: {
...
exports.list_cc_with_limitsOld = (req, res, next) => {
  var query = "1";
  if (req.params.center_id === "all") {
    query = {};
  } else {
...
exports.list_cc_with_limits = (req, res, next) => {
  ...
  if (query != "1") {
    CC.estimatedDocumentCount(query)
      .then((totalRecs) => {
...
exports.fetch_cc = (req, res, next) => {
  CC.find({ _id: req.params.ID })
    .exec()
    .then((data) => {
      res.status(200).json(data);
    })
...
exports.delete_cc = (req, res, next) => {
  // console.log("req.params.ID ",req.params.ID);
  CC.deleteOne({ _id: req.params.ID })
    .exec()
    .then((data) => {
