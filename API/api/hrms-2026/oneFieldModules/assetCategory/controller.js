...
exports.createAssetCategory = async (req, res) => {
  try {
    const existingAssetCategory = await AssetCategoryModal.findOne({
      fieldValue: req.body.fieldValue,
    });

    if (existingAssetCategory) {
      return res.status(409).json({ message: "Asset Category already exists" });
    }

    const assets = new AssetCategoryModal({
...
exports.getAssetCategory = async (req, res) => {
  try {
    const assetCategories = await AssetCategoryModal.find();
...
exports.updateAssetCategory = async (req, res) => {
  ...
  try {
    const assetCategory = await AssetCategoryModal.findById(req.params.id);
    ...
    if (existingAssetCategory) {
      ...
      return res
        .status(409)
        .json({ message: "Asset Category with the same name already exists" });
    }

    let updated = false;
    if (assetCategory.fieldValue !== fieldValue) {
      assetCategory.fieldValue = fieldValue;
      updated = true;
    }
    if (assetCategory.imageName !== imageName) {
      assetCategory.imageName = imageName;
      updated = true;
    }
    if (assetCategory.shortName !== req.body.shortName) {
      assetCategory.shortName = req.body.shortName;
      updated = true;
    }
    ...
    if (updated) {
      ...
      const result = await assetCategory.save();
      try {
        await Promise.all([
...
exports.deleteAssetCategory = async (req, res) => {
  try {
    await AssetCategoryModal.findByIdAndDelete(req.params.id);
...
// exports.bulkUpload_Assets = (req, res, next) => {
...
//       let currentAssets = excelData[k].activity?.trim();

//       if (!currentAssets || currentAssets === "-") {
//         let remark = "Assets not found";
//         invalidData.push({ ...excelData[k], failedRemark: remark });
//         continue;
//       }

//       if (rowSet.has(currentAssets)) {
...
//       rowSet.add(currentAssets);

//       let allAssetss = await getAllAssetss({});
//       let activityExists = allAssetss?.some(
//         (item) => item.fieldValue === currentAssets
//       );

//       if (!activityExists) {
//         validData.push({
//           fieldValue: currentAssets,
//           fileName: req.body?.fileName,
//           createdBy: req.body?.createdBy,
//           createdAt: new Date(),
//         });
//       } else {
//         let remark = "Assets details already exist.";
//         invalidData.push({ ...excelData[k], failedRemark: remark });
//       }
//     }

//     if (validData.length > 0) {
//       AssetsModal.insertMany(validData)
//         .then((data) => {
...
exports.bulkUpload_AssetCategory = (req, res, next) => {
  var excelData = req.body.data;
  var validData = [];
  var invalidData = [];
  var failedRecords = [];
  ...
  async function processData() {
    let allAssetCategories = await getAllAssetCategories({});

    for (var k = 0; k < excelData.length; k++) {
      let currentAssetCategory = excelData[k].activity?.trim();
      let remark = ""; // Reset remark for each row

      // Check if the activity is valid in the file
      if (!currentAssetCategory || currentAssetCategory === "-") {
      ...
      if (rowSet.has(currentAssetCategory)) {
        remark = "Duplicate Asset Category in the file";
        invalidData.push({ ...excelData[k], failedRemark: remark });
        DuplicateCount++;
        continue; // Skip the rest of the loop for this iteration
      }

      rowSet.add(currentAssetCategory);
      ...
      if (!assetCategoryExists) {
        validData.push({
          fieldValue: currentAssetCategory,
          fileName: req.body?.fileName,
          createdBy: req.body?.createdBy,
          createdAt: new Date(),
        });
        ...
        remark = "Asset Category details already exist.";
        invalidData.push({ ...excelData[k], failedRemark: remark });
      }
    }
    ...
    if (validData.length > 0) {
      AssetCategoryModal.insertMany(validData)
        .then((data) => {
          // console.log("Valid data inserted", data);
        })
        .catch((err) => {
...
//   AssetsModal.find({ fileName: req.params.fileName })
//     .exec()
//     .then((data) => {
...
//       if (remark === "") {
//         var allAssetss = await getAllAssetss({});
//         console.log("allAssetss", allAssetss.length);

//         var activityExists = allAssetss?.filter((item) => {
