...
exports.createAssets = async (req, res) => {
  try {
    const existingAssets = await AssetsModal.findOne({
...
exports.getAssets = async (req, res) => {
  try {
    const activitys = await AssetsModal.find();
...
exports.updateAssets = async (req, res) => {
  ...
  try {
    const activity = await AssetsModal.findById(req.params.id);
...
exports.bulkUpload_Assets = (req, res, next) => {
  ...
  async function processData() {
    let allActivities = await getAllAssetss({});
    ...
    for (var k = 0; k < excelData.length; k++) {
      ...
      if (!activityExists) {
        ...
        remark = "Assets details already exist.";
        invalidData.push({ ...excelData[k], failedRemark: remark });
      }
    }
